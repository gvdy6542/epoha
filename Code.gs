...
function rateLimitOk_(id,ctx){
  ctx = ctx || {};
  const {isCallback,hasState,text} = ctx;
  // не ограничаваме активни wizard-и или callback-и, както и основните команди
  if(isCallback || hasState || ['➕ Приход','➖ Разход','/start'].includes(String(text||''))) return true;
  ...
  if(hit) return false;
  cache.put(key,'1',5); // по-кратък TTL за диагностика
  return true;
}
...
function doPost(e) {
  try {
    ...
    Logger.log(`doPost chatId=${chatId} text=${text} data=${data} state=${JSON.stringify(state)}`);
    ...
    if (!isAdmin_(chatId) && !rateLimitOk_(chatId,{isCallback:!!data,hasState:!!state,text})) return ContentService.createTextOutput('ok');
